...
void main(void) {
    ...
    while (1) {
        ...
        lcd_cmd(L_CLR);
        lcd_cmd(L_L1); //Ustawienie karetki w pierwszej linii
        lcd_str("|  TYLKO DZIS  |"); //napis
        
        delay(500);
        lcd_cmd(L_CLR);
        lcd_cmd(L_L1); //Ustawienie karetki w pierwszej linii
        lcd_str("| Dwie   galki |"); //napis
        lcd_cmd(L_L2); //Przej??ie do drugiej linii
        lcd_str("|    lodow     |");
